...
# Read headers
r1 = service.spreadsheets().values().get(spreadsheetId=SHEET_ID, range='Ventas!1:1').execute()
print('Row 1:', r1.get('values', []))

# Read formulas in row 1
r1f = service.spreadsheets().values().get(spreadsheetId=SHEET_ID, range='Ventas!1:1', valueRenderOption='FORMULA').execute()
print('Formulas:', r1f.get('values', []))

# Read a few data rows
rows = service.spreadsheets().values().get(spreadsheetId=SHEET_ID, range='Ventas!A:K').execute()
print('\nAll data:')
